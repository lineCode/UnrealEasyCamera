...
class EASYCAMERA_API UCraneFollow : public UECameraComponentFollow
{
	GENERATED_BODY()
...
protected:
	...
	UPROPERTY(EditAnywhere, BlueprintReadWrite, meta = (EditCondition = "FollowType == ECraneFollowType::FollowTarget"))
	...
	UPROPERTY(EditAnywhere, BlueprintReadWrite, meta = (EditCondition = "FollowType == ECraneFollowType::FixedSpeed || FollowType == ECraneFollowType::FollowTarget"))
	...
	UPROPERTY(EditAnywhere, BlueprintReadWrite, meta = (ClampMin = "0.0", EditCondition = "FollowType == ECraneFollowType::FixedSpeed"))
	FVector Duration;

	/** Speed (per second) when FollowType is FixedSpeed. Respectively for arm length, crane yaw and crane pitch, in world space. */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, meta = (EditCondition = "FollowType == ECraneFollowType::FixedSpeed"))
	FVector Speed;

	/** A set of key points on crane when FollowType is Manual.  */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, meta = (EditCondition = "FollowType == ECraneFollowType::Manual"))
